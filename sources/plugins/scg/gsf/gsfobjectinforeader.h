...
#ifndef GSFOBJECTINFOREADER_H_
#define GSFOBJECTINFOREADER_H_
...
{
public:
    Node(const QPair<QString, QString>& data)
        : type(data.first), idtf(data.second) {}
    Node(const QString& type, const QString& idtf)
        : type(type), idtf(idtf) {}
...
public:
    Edge()
        : first("",""), second("","") {}
    Edge(const QString& type, const QString& idtf, const Node& first, const Node& second)
        : type(type), idtf(idtf), first(first), second(second) {}
    Edge(const QPair<QString, QString>& data, const Node& first, const Node& second)
        : type(data.first), idtf(data.second), first(first), second(second) {}
...
public:
     ...
     */
    GsfObjectInfoReader(bool isOwner = true);

    //! Automaticaly calls method \ref read() for document @p document.
    explicit GsfObjectInfoReader(const QDomDocument& document, bool isOwner = true);

    virtual ~GsfObjectInfoReader();
...
private:
    ...
    GSFType2TypeAliasMap mGSFType2TypeAlias;
    ...
    QMap<QString, Edge> parseScsSector(const QDomElement& scsSector);

    QPair<QString, QString> parseTypeIdent(const QString& str);
    ...
    QMap<QString, QString> mIdtf2Id;

    QMap<QString, Edge> mEdges;
};

#endif /* GSFOBJECTINFOREADER_H_ */
